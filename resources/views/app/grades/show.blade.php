                            ...
                            <div>
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb mb-1 extra-small text-uppercase fw-bold ls-1">
                                        <li class="breadcrumb-item"><a href="{{ route('grades.index') }}" class="text-white text-opacity-75 text-decoration-none">Registre</a></li>
                                        <li class="breadcrumb-item active text-white" aria-current="page">Fiche Grade</li>
                                    </ol>
                                </nav>
                                <h1 class="fw-bold mb-0 display-6">{{ $grade->title }}</h1>
                            </div>
                        </div>
                        <div>
                            <a href="{{ route('grades.index') }}" class="btn btn-glass-light btn-rounded px-4 py-2 fw-bold shadow-sm transition-base">
                                <i class="bi bi-arrow-left-short fs-5 me-1"></i>Retour au référentiel
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div class="card border-0 shadow-sm rounded-4 h-100 bg-white">
                    <div class="card-body p-4 p-xl-5">
                        <div class="d-flex align-items-center mb-4">
                            ...
                            <h5 class="fw-bold text-dark mb-0">Paramétrage</h5>
                        </div>

                        <form action="{{ route('grades.update', $grade->id) }}" method="POST">
                            @csrf
                            <div class="mb-4">
                                <label for="serviceTitle" class="form-label small fw-bold text-muted text-uppercase ls-1">Nom Officiel</label>
                                <div class="input-group-futurist">
                                    <input type="text" id="serviceTitle" name="title" class="form-control futurist-input" value="{{ old('title', $grade->title) }}" required>
                                    <i class="bi bi-award input-icon"></i>
                                </div>
                            </div>

                            <div class="mb-4">
                                <label class="form-label small fw-bold text-muted text-uppercase ls-1">Échelle Indiciaire</label>
                                <div class="input-group-futurist">
                                    <select name="scale" class="form-select futurist-input fw-bold text-primary" required>
                            ...
                            <button type="submit" class="btn btn-futurist w-100 rounded-pill fw-bold py-3 transition-base mt-2">
                                <i class="bi bi-save2 me-2"></i>Mettre à jour le grade
                            </button>
                        </form>
                        ...
                        <div class="stats-panel mt-5">
                            <div class="row g-0 rounded-4 overflow-hidden border">
                                <div class="col-6 border-end p-3 text-center">
                                    ...
                                    <div class="h4 fw-bold text-primary mb-0">{{ $grade->competences->count() }}</div>
                                </div>
                                <div class="col-6 p-3 text-center bg-light-subtle">
                                    <div class="text-muted extra-small fw-bold text-uppercase mb-1">Index</div>
                                    <div class="h4 fw-bold text-success mb-0">STAT-{{ $grade->scale }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-lg-8">
                <div class="card border-0 shadow-sm rounded-4 bg-white h-100">
                    <div class="card-header bg-transparent py-4 px-4 border-bottom-0 d-flex justify-content-between align-items-center">
                        <div>
                            <h5 class="mb-1 fw-bold text-dark">Titulaires du Grade</h5>
                            <p class="text-muted small mb-0">Agents classés sous cette échelle statutaire</p>
                        </div>
                        <span class="badge-cyber">
                            {{ $grade->competences->count() }} Collaborateurs
                        </span>
                    </div>
                    <div class="card-body p-4">
                        ...
                        @else
                            <div class="text-center py-5">
                                ...
                                <h5 class="text-dark fw-bold">Aucun titulaire</h5>
                                <p class="text-muted small mx-auto" style="max-width: 300px;">
                                    Ce grade ne comporte actuellement aucun agent actif dans la base de données.
                                </p>
                            </div>
                        @endif
                    </div>
