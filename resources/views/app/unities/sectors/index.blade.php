<x-layout>
    ...
    <div class="container-fluid py-4 px-md-5">
                ...
                <div class="col-md-7">
                    <nav aria-label="breadcrumb" class="mb-2">
                        <ol class="breadcrumb mb-0 extra-small text-uppercase fw-bold ls-1">
                            ...
                            <li class="breadcrumb-item active text-primary" aria-current="page">Secteurs</li>
                        </ol>
                    </nav>
                    <h1 class="fw-bold text-dark display-6 mb-1">Architecture des <span class="text-primary-gradient">Secteurs</span></h1>
                    <p class="text-muted mb-0">Management des sous-unités et des responsables opérationnels</p>
                </div>
                <div class="col-md-5 text-md-end mt-4 mt-md-0">
                    <div class="d-flex justify-content-md-end gap-2">
                        <a class="btn btn-glass shadow-sm rounded-pill px-4 fw-bold transition-base" href="{{ route('sectors.download') }}">
                            <i class="bi bi-cloud-arrow-down me-2"></i>Export
                        </a>
                        <a href="{{ route('sectors.create') }}" class="btn btn-futurist shadow-lg rounded-pill px-4 fw-bold transition-base">
        ...
        <div class="row g-4 mb-5">
            @php
                $stats = [
                    ['label' => 'Secteurs', 'count' => $sectors->total(), 'color' => '#6366f1', 'icon' => 'bi-grid-1x2'],
                    ['label' => 'Entités', 'count' => $total_entity, 'color' => '#10b981', 'icon' => 'bi-diagram-2-fill'],
                    ['label' => 'Sections', 'count' => $total_section, 'color' => '#0ea5e9', 'icon' => 'bi-layers'],
                    ['label' => 'Service', 'count' => $total_service, 'color' => '#f59e0b', 'icon' => 'bi-diagram-3-fill']
                ];
            @endphp
                        ...
                        <form method="GET" action="{{ route('sectors.index') }}">
                            <div class="mb-3">
                                ...
                                <div class="input-group-futurist">
                                    <input type="text" name="search" value="{{ $filter }}" class="form-control futurist-input" placeholder="Secteur ou responsable...">
                            ...
                            <button type="submit" class="btn btn-dark w-100 rounded-3 py-2 fw-bold shadow-sm transition-base mt-2">
                                Actualiser
                            </button>
                            <a href="{{ route('sectors.index') }}" class="btn btn-link w-100 text-decoration-none text-muted small mt-2 text-center">Réinitialiser</a>
                        </form>
                    </div>
                </div>
            </div>
                ...
                <div class="card border-0 shadow-lg rounded-4 overflow-hidden bg-white">
                    <div class="card-header bg-white py-4 px-4 border-bottom-0 d-flex justify-content-between align-items-center">
                        <h5 class="mb-0 fw-bold text-dark d-flex align-items-center">
                            Registre des Secteurs
                            <span class="badge-cyber active ms-3">{{ $sectors->total() }} secteurs</span>
                        </h5>
                    </div>
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0">
                            ...
                            <tr class="bg-light-subtle">
                                <th class="ps-4 py-3 text-muted small text-uppercase ls-1 fw-bold border-0">Secteur Node</th>
                                <th class="py-3 text-muted small text-uppercase ls-1 fw-bold border-0">Responsable / Chef</th>
                                <th class="py-3 text-muted small text-uppercase ls-1 fw-bold border-0">Hiérarchie</th>
                                <th class="pe-4 py-3 text-muted small text-uppercase ls-1 fw-bold border-0 text-end"></th>
                            </tr>
                            </thead>
                            <tbody>
                                ...
                                <tr class="hover-row transition-base">
                                    <td class="ps-4 py-4">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                <div class="fw-bold text-dark fs-6">{{ $sector->title }}</div>
                                                <code class="extra-small text-primary">ID: #SEC-{{ str_pad($sector->id, 3, '0', STR_PAD_LEFT) }}</code>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="py-4">
                                        ...
                                        @else
                                            <span class="management-pill empty text-muted italic extra-small">
                                                    <i class="bi bi-person-x me-2"></i>Poste non pourvu
                                                </span>
                                        @endif
                                    </td>
                                    <td class="py-4">
                                        <div class="d-flex flex-column gap-1">
                                                <span class="badge-node">
                                                    <i class="bi bi-diagram-3-fill me-1 text-primary"></i>{{ $sector->entity->title ?? 'Indépendant' }}
                                                </span>
                                            <span class="extra-small fw-bold text-muted opacity-75">
                                                    <i class="bi bi-chevron-right ms-1"></i> {{ $sector->affectations->count() }} Agents
                                                </span>
                                        </div>
                                    </td>
                                    <td class="pe-4 py-4 text-end">
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end shadow-lg border-0 rounded-4 p-2">
                                                <li><a class="dropdown-item rounded-3 py-2" href="{{ route('sectors.show', $sector) }}"><i class="bi bi-eye-fill me-2 text-primary"></i>Consulter</a></li>
                                                <li><a class="dropdown-item rounded-3 py-2" href="{{ route('sectors.edit', $sector) }}"><i class="bi bi-pencil-fill me-2 text-success"></i>Editer</a></li>
                                                <li><hr class="dropdown-divider opacity-50"></li>
                                                <li><button class="dropdown-item rounded-3 py-2 text-danger fw-bold" data-bs-toggle="modal" data-bs-target="#deleteSectorModal-{{ $sector->id }}"><i class="bi bi-trash3 me-2"></i>Retirer</button></li>
                                            </ul>
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center py-5">
                                        ...
                                        <h6 class="text-muted fw-bold">Structure sectorielle vide</h6>
                                    </td>
                                </tr>
                            @endforelse
                            </tbody>
                    ...
                    @if(isset($sectors) && $sectors->hasPages())
                        <div class="card-footer bg-white border-top-0 py-4 px-4">
                            <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                                <div class="text-muted small">Vue {{ $sectors->firstItem() }} - {{ $sectors->lastItem() }} sur {{ $sectors->total() }}</div>
    ...
    <style>
        ...
        .icon-shape-futur {
            width: 42px; height: 42px; background: #f1f5f9; color: #6366f1;
            display: flex; align-items: center; justify-content: center;
            border-radius: 12px; transition: all 0.3s ease;
        }
        .hover-row:hover .icon-shape-futur { background: var(--primary-gradient); color: white; transform: scale(1.1); }
