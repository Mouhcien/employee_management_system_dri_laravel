<x-layout>
    @section('title', 'Détails Diplôme - ' . $diploma->title)
                            ...
                            <div>
                                <nav aria-label="breadcrumb">
                                    <ol class="breadcrumb mb-1 extra-small text-uppercase fw-bold ls-1">
                                        <li class="breadcrumb-item"><a href="{{ route('diplomas.index') }}" class="text-white text-opacity-75 text-decoration-none">Référentiel</a></li>
                                        <li class="breadcrumb-item active text-white" aria-current="page">Fiche Certification</li>
                                    </ol>
                                </nav>
                                <h1 class="fw-bold mb-0 display-6 text-truncate" style="max-width: 600px;">{{ $diploma->title }}</h1>
                            </div>
                        </div>
                        <div>
                            <a href="{{ route('diplomas.index') }}" class="btn btn-glass-light btn-rounded px-4 py-2 fw-bold shadow-sm transition-base">
                ...
                <div class="card border-0 shadow-sm rounded-4 h-100 bg-white">
                    <div class="card-body p-4 p-xl-5">
                        <div class="d-flex align-items-center mb-4">
                            ...
                            <h5 class="fw-bold text-dark mb-0">Paramètres</h5>
                        </div>

                        <form action="{{ route('diplomas.update', $diploma->id) }}" method="POST">
                            @csrf
                            <div class="mb-4">
                                <label for="serviceTitle" class="form-label small fw-bold text-muted text-uppercase ls-1">Intitulé Académique</label>
                                ...
                                <div class="form-text extra-small mt-3 text-muted">
                                    <i class="bi bi-info-circle me-1"></i> Impacte <strong>{{ $diploma->qualifications->count() }}</strong> profils employés.
                                </div>
                            </div>

                            <button type="submit" class="btn btn-futurist w-100 rounded-pill fw-bold py-3 transition-base">
                                <i class="bi bi-arrow-repeat me-2"></i>Mettre à jour la filière
                            </button>
                        </form>
                        ...
                        <div class="stats-panel mt-5">
                            <div class="row g-0 rounded-4 overflow-hidden border">
                                <div class="col-6 border-end p-3 text-center">
                                    <div class="text-muted extra-small fw-bold text-uppercase mb-1">Titulaires</div>
                                    <div class="h3 fw-bold text-primary mb-0">{{ $diploma->qualifications->count() }}</div>
                                </div>
                                <div class="col-6 p-3 text-center bg-light-subtle">
                                    <div class="text-muted extra-small fw-bold text-uppercase mb-1">Impact</div>
                                    <div class="h3 fw-bold text-info mb-0">Haut</div>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div class="col-lg-8">
                <div class="card border-0 shadow-sm rounded-4 bg-white h-100">
                    <div class="card-header bg-transparent py-4 px-4 border-bottom-0 d-flex justify-content-between align-items-center">
                        <div>
                            <h5 class="mb-1 fw-bold text-dark">Employés Diplômés</h5>
                            <p class="text-muted small mb-0">Collaborateurs possédant cette certification</p>
                        </div>
                        <span class="badge-cyber">
                            {{ $diploma->qualifications->count() }} Profils
                        </span>
                    </div>
                    <div class="card-body p-4">
                        ...
                        @else
                            <div class="text-center py-5">
                                ...
                                <h5 class="text-dark fw-bold">Aucune affectation</h5>
                                <p class="text-muted small mx-auto" style="max-width: 350px;">
                                    Aucun membre de l'organisation n'est actuellement rattaché à ce diplôme spécifique dans le registre.
                                </p>
                            </div>
                        @endif
                    </div>
