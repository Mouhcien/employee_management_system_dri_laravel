                    ...
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 position-relative">
                        <div class="d-flex align-items-center">
                            <div class="bg-white bg-opacity-20 rounded-3 p-3 me-3 shadow-sm">
                                <i class="bi bi-patch-check-fill fs-3 text-dark"></i>
                            </div>
                            <div>
                                <h2 class="fw-bold mb-1">Diplôme : {{ $diploma->title }}</h2>
                                <p class="text-white text-opacity-75 mb-0 small fw-medium">
                                    <i class="bi bi-info-circle me-1"></i>
                                    Gestion du référentiel académique et des qualifications
                                </p>
                            </div>
                        </div>
                        <a href="{{ route('diplomas.index') }}" class="btn btn-white btn-rounded px-4 fw-bold shadow-sm transition-base">
                ...
                <div class="card border-0 shadow-sm rounded-4 h-100">
                    <div class="card-header bg-white border-bottom-0 pt-4 px-4">
                        <h6 class="fw-bold text-uppercase text-muted small mb-0 ls-1">
                            <i class="bi bi-gear-wide-connected text-primary me-2"></i>Paramètres de la filière
                        </h6>
                    </div>
                    <div class="card-body p-4">
                        <form action="{{ route('diplomas.update', $diploma->id) }}" method="POST">
                            @csrf
                            <div class="mb-4">
                                <label for="serviceTitle" class="form-label small fw-bold text-dark text-uppercase ls-1">Intitulé officiel</label>
                                ...
                                <div class="form-text extra-small mt-2">Ce nom sera mis à jour sur toutes les fiches employés liées à cette filière.</div>
                            </div>

                            <button type="submit" class="btn btn-primary w-100 rounded-pill fw-bold shadow-sm py-2 transition-base">
                                <i class="bi bi-save2 me-2"></i>Mettre à jour l'intitulé
                            </button>
                        </form>

                        <div class="mt-4 p-3 bg-primary-subtle rounded-4 border border-primary-subtle border-opacity-25 text-center">
                            <div class="small fw-bold text-primary mb-1 text-uppercase ls-1">Taux de qualification</div>
                            <h3 class="fw-bold text-primary mb-0">{{ $diploma->qualifications->count() }}</h3>
                            <span class="text-primary text-opacity-75 extra-small">Employés diplômés</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card border-0 shadow-sm rounded-4 h-100">
                    <div class="card-header bg-white py-4 px-4 border-bottom d-flex justify-content-between align-items-center">
                        ...
                        <div class="bg-light px-3 py-1 rounded-pill border small fw-bold text-muted">
                            Total : {{ $diploma->qualifications->count() }}
                        </div>
                    </div>
                    <div class="card-body p-4">
                        ...
                        @else
                            <div class="text-center py-5">
                                ...
                                <h6 class="text-muted fw-bold">Aucun diplômé détecté</h6>
                                <p class="text-muted small px-5">Il n'y a actuellement aucun employé rattaché à ce diplôme spécifique.</p>
                            </div>
                        @endif
                    </div>
                </div>
