<x-layout>
    @section('title', 'Gestion des cities - HR Management')
    ...
    @section('content')
        <div class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 class="text-2xl font-semibold text-gray-900">Gestion des cities</h1>
                    <p class="mt-1 text-sm text-gray-500">Gérer vos cities</p>
                </div>
            ...
            <div class="bg-white rounded-lg shadow p-4">
                <form method="GET" action="{{ route('cities.index') }}" class="flex flex-col md:flex-row gap-4">
                    <div class="flex-1">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Locals</th>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900">{{ $city->title }} </div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">
                                        @forelse($city->locals as $local)
                                            <div class="text-sm text-gray-900">{{ $local->title }}</div>
                                        @empty
                                            <a>Ajouter nouveau local</a>
                                        @endforelse
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2" x-data="{ open: false }">
                                        <a href="{{ route('cities.show', $city) }}" class="text-indigo-600 hover:text-indigo-900" title="View">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                        </a>
                                        <a href="{{ route('cities.edit', $city) }}" class="text-indigo-600 hover:text-indigo-900" title="Edit">
                                        ...
                                        <div class="relative">
                                            ...
                                            <div x-show="open" class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-10">
                                                ...
                                                <form method="POST" action="{{ route('cities.delete', $city) }}" onsubmit="return confirm('Are you sure you want to delete this employee?');">
                                                    @csrf
                                                    @method('DELETE')
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <h3 class="text-lg font-medium text-gray-900 mb-1">No cities found</h3>
                                        <p class="text-gray-500 mb-4">Get started by creating your first employee record.</p>
                                        <a href="{{ route('cities.create') }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
